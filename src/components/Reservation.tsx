...
export const Reservation = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const message =
      `🍽️ *New Reservation at ${businessName}*\n\n` +
      `👤 *Name:* ${formData.name}\n` +
        ...
        >
          {formState === 'success' ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-gray-400">We look forward to hosting you at {businessName}.</p>
              <p className="text-gray-500 text-sm mt-2">Your details were sent via WhatsApp.</p>
              <button
                type="button"
                onClick={() => { setFormState('idle'); setFormData({ name: '', phone: '', date: '', time: '', guests: '' }); }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="relative group md:col-span-2">
                ...
                <input type="number" name="guests" value={formData.guests} onChange={handleChange} min="1" max="12" placeholder="Number of Guests" required
                  className="w-full bg-dark-500/50 border border-white/10 rounded-lg py-4 pl-12 pr-4 text-white placeholder-gray-500 focus:outline-none focus:border-brand-500/50 transition-colors" />
              </div>
