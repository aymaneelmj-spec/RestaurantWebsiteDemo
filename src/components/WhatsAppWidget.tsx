...
export const WhatsAppWidget = () => {
  const { phone, businessName } = useDemo();
  ...
  const waUrl = (text: string) => `https://wa.me/${phone}?text=${encodeURIComponent(text)}`;

  const toggleItem = (id: number) => {
    setSelectedItems(prev => {
      const existing = prev.find(item => item.id === id);
      return existing ? prev.filter(item => item.id !== id) : [...prev, { id, qty: 1 }];
    });
  };
  ...
  // First letter of business name for avatar
  const avatarLetter = businessName.charAt(0);
          ...
          >
            ...
            <div className="bg-brand-500 p-4 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-medium text-sm">{businessName} Assistant</h3>
                  <p className="text-xs text-brand-100 opacity-90 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-green-400"></span> Online
                  </p>
                </div>
              </div>
              <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-white/20 rounded-full transition-colors" aria-label="Close Chat">
                <X className="w-5 h-5" />
              </button>
            </div>

            {/* Chat Body */}
            <div className="flex-1 p-4 overflow-y-auto max-h-[400px] flex flex-col minimal-scrollbar bg-[#1c1c1f]">

              {step === 'initial' && (
                <>
                  <Message>Hello! Welcome to {businessName} 🌴</Message>
                  <Message>Are you looking to book a table or order food?</Message>
                  <div className="flex flex-col gap-2 mt-2 px-2">
                    <button onClick={() => setStep('book')}
                      className="bg-dark-400 hover:bg-brand-500/20 hover:text-brand-400 hover:border-brand-500/50 transition-all border border-white/10 text-white rounded-xl py-3 px-4 text-sm font-medium text-left">
                      📅 Book a Table
                    </button>
                ...
                <>
                  ...
                  <form onSubmit={handleBookSubmit} className="flex flex-col gap-3 mt-2 px-2">
                    ...
                    <div className="relative">
                      ...
                      <input type="number" min="1" required value={guests} onChange={(e) => setGuests(e.target.value)} placeholder="Guests"
              ...
              {step === 'order' && (
                <>
                  ...
                  <Message>Great choice! Select items below and send your order via WhatsApp.</Message>
