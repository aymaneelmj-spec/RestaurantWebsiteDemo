    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-12 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <span className="font-serif text-3xl tracking-widest text-brand-500 font-bold drop-shadow-sm leading-none">
            {businessName}
          </span>
          <span className="text-[10px] uppercase tracking-[0.2em] text-gray-400 mt-1 hidden sm:block border-l border-white/20 pl-2 ml-2">
            Qatari <br/>Cuisine
          </span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button onClick={() => setLangSelector(!langSelector)} className="p-2 rounded-full hover:bg-white/5 transition-colors text-gray-300 hover:text-brand-400">
      ...
      <AnimatePresence>
        {mobileMenu && (
          ...
          >
            <button className="absolute top-6 right-6 p-2 text-white hover:text-brand-400 transition-colors" onClick={() => setMobileMenu(false)}>
              <X className="w-8 h-8"/>
            </button>
